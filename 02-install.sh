...
HOOKS=(base udev autodetect modconf block keyboard sd-vconsole zfs filesystems)
COMPRESSION="zstd"
EOF
...
systemd-nspawn -D /mnt -- /usr/bin/systemctl enable systemd-timesyncd.service systemd-time-wait-sync.service
systemd-nspawn -D /mnt -- /usr/bin/systemctl enable iwd.service systemd-networkd.service systemd-resolved.service

_gfxidentifier="$(lspci | grep -e VGA -e 3D)"
    declare -r _gfxidentifier
    case "$_gfxidentifier" in
    #*\ Intel\ *) pacstrap /mnt xf86-video-intel ;; # disabled following https://github.com/Finkregh/archstrap/pull/10#discussion_r427403432
    *\ NVIDIA\ *) pacstrap /mnt xf86-video-nouveau ;;
